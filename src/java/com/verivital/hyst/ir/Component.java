...
public abstract class Component {
	...
	public void validate() {
		...
		if ((instanceName == null) != (parent == null))
			throw new AutomatonValidationException(
					"instanceName was " + (instanceName == null ? "<null>" : instanceName) + " but parent was "
							+ (parent == null ? "<null>" : "not null")
		...
		if (VALIDATE_CONSTS) {
			...
			for (String cName : constants.keySet()) {
				try {
					...
					throw new AutomatonValidationException(
							"Constant '" + cName + "' in automaton " + getPrintableInstanceName() + " is unmapped", e);
				}
			}
		}
	}
		...
		if (val == null) {
			...
			if (val == null)
				throw new AutomatonExportException(
						"Unassigned constant: '" + name + "' in instance '" + getPrintableInstanceName() + "'");
		}
	...
	/**
	 * Check if a name is valid. C identifier names are allowed (first letter is '_'
	 * or letter, remaining letters can be alphanumeric or '_')
	 * 
	...
	/**
	 * Check for name conflicts (and valid names) in variables, constants, labels.
	 * This accumulates names into allNames as it goes, and reports conflicts as
	 * they come up
	 * 
	 * @param allNames the accumulator which checks for collisions
	 * @param newNames the new names
	 */
	public static void checkNameConflicts(HashSet<String> allNames, Collection<String> newNames, String componentName) {
	...
	/**
	 * get the names of all the variables (include locals in subcomponents) in this
	 * component
	 */
	...
	/**
	 * Get the fully-qualified instance name (with dots). For root component this is
	 * the empty string
	 * 
	...
	/**
	 * Get the fully-qualified variable name (with dots). For root component this is
	 * just the variable name. If there's a parent the name may be remapped.
	 * 
